...
'''
Source: Ma H, Han S, Lei H. Optimized paillier’s cryptosystem with fast encryption and decryption[C]//Annual Computer Security Applications Conference. 2021: 106-118.
'''


def KGen_NewOpt(k=112, sigma=128, yita=0, secret_key_1=None):
    '''
    the algorithm of key generation
    :param k: secure parameter (secure level)
    :param sigma: the bit-length of private key 1
    :param yita: the value η
    :param bit_len_of_plaintext: the scope of plaintext
    :param secret_key_1: users are able to pass in a private key 1
    :return: private_key is the private key，public_key is the public key，private_key_1 is private key 1，private_key_2 private key 2
    '''
    # step1
    N, P, Q, p, q = Ngen(k)
    # step2
    ...
    public_key = {
        ...
        'L_k': get_L_k(k),
        'sigma': sigma,
        'table': None,  # pre-computation table for speeding up encryption
        'L': libnum.randint_bits(sigma + 2)  # constant L
    }
...
def Enc_NewOpt(public_key, m):
    ...
    if m < 0:
        # transfer the negative value into the positive one
        m += N

    r = random.randint(0, 2 ** L_k)
    N_square = gmpy2.mpz(N * N)

    if table == None:  # have no pre-computation table
...
def Dec_NewOpt(private_key, cipher_text):
    ...
    # recover the value into negative one
    if m > N // 2:
        m = m - N
    return m
...
def PDec(partial_private_key, ciphertext):
    '''
    partial decryption
    :param partial_private_key: partial key
    :param ciphertext:
    :return: the result of partial decryption
    '''
...
def TDec(ciphertext1, ciphertext2, N):
    ...
    :param ciphertext2:
    :param N:
    :return: the result of threshold decryption
    '''
...
def get_n_k(k=112):
    '''
    get the bit-length of N, according to the secure parameter
    :param k:
    :return:
    '''
    if k == 64:
...
def get_L_k(k=112):
    '''
    get the bit-length of private key, according to the secure parameter
    :param k:
    :return:
    '''
    return 4 * k
...
def is_co_prime(p, q, p_another, q_another):
    '''
    Determine whether p、q、p'、q' are co-prime
    :param p:
    :param q:
    :param p_another:p'
    :param q_another:q'
...
def get_odd_len_integer(len):
    '''
    get an odd number with len bits
    :param len:
    :return:
    '''
    while True:
